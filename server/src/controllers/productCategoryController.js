...
class ProductCategoryController {
    ...
    createProductCategory = asyncHandler(async (req, res) => {
        ...
        res.status(200).json({
            ...
            data: newProductCategory ? newProductCategory : "Cannot create product category",
        });
    });
    ...
    getAllProductCategories = asyncHandler(async (req, res) => {
        ...
        res.status(200).json({
            ...
            data: productCategories ? productCategories : "Cannot get product categories",
        });
    });
    ...
    updateProductCategory = asyncHandler(async (req, res) => {
        ...
        const updateProductCategory = await ProductCategory.findByIdAndUpdate(req.params.pcid, req.body, { new: true });
        res.status(200).json({
            status: updateProductCategory ? true : false,
            data: updateProductCategory ? updateProductCategory : "Cannot update product category",
        });
    });

    // DELETE : delete a product category   
    deleteProductCategory = asyncHandler(async (req, res) => {
        const deleteProductCategory = await ProductCategory.findByIdAndDelete(req.params.pcid);
        res.status(200).json({
            status: deleteProductCategory ? true : false,
            data: deleteProductCategory ? deleteProductCategory : "Cannot delete product category",
        });
    });
}
